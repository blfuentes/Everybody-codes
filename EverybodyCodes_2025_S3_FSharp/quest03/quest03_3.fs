...
let rec connectPart (i: int) (child: int byref) (nodes: ResizeArray<TreeNode>) : bool =
    let leftDone =
        match nodes[i].LeftChild with
        | Some leftChild ->
            if not (matchesStrong nodes[i].Node.LeftSocket nodes[leftChild].Node.Plug)
               && matchesStrong nodes[i].Node.LeftSocket nodes[child].Node.Plug then
                nodes[i].LeftChild <- Some child
                child <- leftChild
                false
            else
                connectPart leftChild &child nodes
        | None ->
    ...
    if leftDone then true
        ...
        | Some rightChild ->
            if not (matchesStrong nodes[i].Node.RightSocket nodes[rightChild].Node.Plug)
               && matchesStrong nodes[i].Node.RightSocket nodes[child].Node.Plug then
                nodes[i].RightChild <- Some child
                child <- rightChild
                false
            else
                connectPart rightChild &child nodes
        | None ->
...
let execute() =
    ...
    for node in parsed do
        ...
        if pos > 0 then
            let mutable child = pos
            let mutable inserted = false
            while not inserted do
                inserted <- connectPart 0 &child treeNodes
