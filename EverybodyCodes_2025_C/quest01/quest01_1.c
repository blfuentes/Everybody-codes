...
char* execute_quest01_1() {
	/*const char* filename = "quest01/test_input_1.txt";*/
	const char* filename = "quest01/quest01_input_1.txt";
	...
	for (int i = 0; i < operators_count; i++) {
		Operator op = ops[i];
		if (op.symbol == 'L') {
			current_position = max(current_position - op.distance, 0);
		}
		else {
			current_position = min(current_position + op.distance, name_count - 1);
		}
	}
