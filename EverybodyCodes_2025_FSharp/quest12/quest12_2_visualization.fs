module quest12_2_visualization
...
let igniteBarrels(barrelsMap: Barrel[,]) =
    ...
    Set.union affectedPositions affectedPositionsFromSecond
...
let visualizeBurntBarrels (barrelsMap: Barrel[,]) (burntBarrels: Set<Position>) =
    ...
    saveBarrelsImage (Path.Combine(VisualizationFolder, "quest12_2_visualization.png")) bitmap

let execute() =
    let lines = LocalHelper.GetLinesFromFile(path)
    let barrelsMap = parseContent lines
    let burntBarrls = igniteBarrels barrelsMap
    visualizeBurntBarrels barrelsMap burntBarrls
    burntBarrls.Count
