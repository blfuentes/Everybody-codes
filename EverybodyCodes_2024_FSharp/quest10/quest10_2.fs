module quest10_part02

open EverybodyCodes_2024_FSharp.Modules
open System.Collections.Generic

//let path = "quest10/test_input_02.txt"
let path = "quest10/quest10_input_02.txt"
...
let parseFull (lines: string[]) =
    let blockHeight = (lines.Length + 1) / 9
    let blockWidth = (lines[0].Length + 1) / 9
                ...
                for col in 0..blockWidth - 1 do
                    ...
                    let toFill =
                        seq {
                            for rIdx in 0..7 do
                                for cIdx in 0..7 do
                                    let char = lines[rIdx + row * 9][cIdx + col * 9]
...
let execute() =
    ...
    |> List.map(fun runeDef ->
        ...
        let runes = 
            toFill
            |> Seq.map (fun pos -> findRuneLocal pos)
            |> Seq.toArray
