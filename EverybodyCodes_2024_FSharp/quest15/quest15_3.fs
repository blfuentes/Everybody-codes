...
// split a POI into separate labeled points
let applySplit (poi: POI) (path: Path) (split: string) =
    if poi.ContainsKey split then
        let locs = poi[split] |> Seq.toList
        poi.Remove split |> ignore
        for i, loc in Seq.indexed locs do
            let key = split + string i
            poi[key] <- HashSet([loc])
            path[loc] <- key

// choose one location among candidates
let applyChoice (poi: POI) (path: Path) (start: string) (choose: HashSet<Coord> -> Coord) =
    if poi.ContainsKey start then
        let locs = poi[start]
        // reset all to "."
        for loc in locs do
            path[loc] <- "."
        // pick one
        let choice = choose locs
        poi[start] <- HashSet([choice])
        path[choice] <- start
...
let findBestRoute (data: string list) (start: string) (split: string option) (choose: (HashSet<Coord> -> Coord) option) =
    let poi, path = parseContent data

    // apply split if requested
    match split with
    | Some s -> applySplit poi path s
    | None -> ()

    // apply choice if requested
    match choose with
    | Some chooser -> applyChoice poi path start chooser
    | None -> ()
...
let execute() =
    ...
    let colWidth = lines.Head.Length / 3

    let left   = lines |> List.map (fun line -> line[0 .. colWidth-1])
    let middle = lines |> List.map (fun line -> line[colWidth .. 2*colWidth-1])
    let right  = lines |> List.map (fun line -> line[2*colWidth .. line.Length-1])

    // Define chooser functions
    let chooseMin (locs: HashSet<Coord>) = locs |> Seq.min
    let chooseMax (locs: HashSet<Coord>) = locs |> Seq.max

    // Run the three subproblems
    let l = findBestRoute left   "E"     None            (Some chooseMax)
    let m = findBestRoute middle "Start" (Some "K")      None
    let r = findBestRoute right  "R"     None            (Some chooseMin)
